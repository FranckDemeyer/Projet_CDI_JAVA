...
public class ProfessionalWebServiceImpl implements ProfessionalWebService {
	...
	public List<ProfessionalDTO> findAllProfessionals() {
		return ServiceFactory.getInstance().getProfessionalService().getAllProfessionals();
	}

	@Override
	public List<ProfessionalDTO> findProfessionalsByName(String name) {
		return ServiceFactory.getInstance().getProfessionalService().getProfessionalsByName(name);
	}

	@Override
	public List<ProfessionalDTO> findProfessionalsByCategory(ProfessionalCategoryDTO category) {
		return ServiceFactory.getInstance().getProfessionalService().getProfessionalsByCategory(category);
	}

	@Override
	public List<ProfessionalDTO> findProfessionalsByCoord(String lat, String lng, double radius) {
		return ServiceFactory.getInstance().getProfessionalService().getProfessionalsByCoord(lat, lng, radius);
	}
